...
namespace RoomReservation.Web.DataTransferModels.Campaign
{
    public class CampaignRequestModel : IValidatableObject
    {
        ...
        {
            if (DateTime.Compare(DateTime.Now, (DateTime)this.Phase1Start) > 0)
            {
                yield return new ValidationResult("Phase 1 start must be in the future!", new string[] { "Phase1Start" });
            }
